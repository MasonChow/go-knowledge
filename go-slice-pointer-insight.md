...
- 内存本质是带编号的储物柜，编号即“地址”。
- 单块大内存：所有内容连续存放，编号顺序排列，CPU 取用快。
- 多块小内存：分成几块，编号可能不连续，CPU 取用时需多次定位。
